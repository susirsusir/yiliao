...
public class AudioRecorderButton extends Button {
    ...
    //时间到达提示
    private static final int MSG_DIALOG_ARRIVE = 0x113;
    //时间到达终止
        ...
        public void run() {
            while (isRecording) {//判断正在录音
                try {
                    ...
                    if (mTime > 20) {
                        mHandler.sendEmptyMessage(MSG_DIALOG_ARRIVE);
                    }
                    if ((int) (30 - mTime) <= 0) {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_DIALOG_DIMISS:
                    ...
                    break;
                case MSG_DIALOG_ARRIVE:
                    mDialogManager.arriveTime(mTime);
                    break;
    ...
    public AudioRecorderButton(Context context, AttributeSet attrs) {
        ...
        //录音文件存放地址
        String dir = Environment.getExternalStorageDirectory() + "/yiliao/voice";
    ...
    private void changeState(int state) {
        if (mCurrentState != state) {
            ...
            switch (state) {
                case STATE_NORMAL:
                    setBackgroundResource(R.drawable.ic_voice);
                    setTitleText(R.string.chat_voice_let_start);
                    break;

                case STATE_RECORDING:
                    setBackgroundResource(R.drawable.audio_anim);
                    AnimationDrawable animationDrawable1 = (AnimationDrawable) getBackground();
                    animationDrawable1.start();
                    setTitleText(R.string.chat_voice_let_end);
                    if (isRecording) {
                ...
                case STATE_CANCEL:
                    ...
                    setTitleText(R.string.chat_voice_let_cancel);
                    break;
            }
        }
    }
    ...
    public void setTitle(TextView hint) {
        this.mTvHint = hint;
    }

    private void setTitleText(int stringId) {
        if (mTvHint != null) {
            mTvHint.setText(stringId);
        }
    }
}
